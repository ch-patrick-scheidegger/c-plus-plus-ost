...
auto execute_measurement(std::vector<int> const &values, std::ostream &out) -> void {
  ...
  auto const loop_duration =
      std::chrono::duration_cast<std::chrono::milliseconds>(loop_end -
                                                            loop_start);
  out << "found with a loop: " << loop_result << " in " << loop_duration.count()
      << "ms" << std::endl;
  ...
  auto const algorithm_duration =
      std::chrono::duration_cast<std::chrono::milliseconds>(algorithm_end -
                                                            algorithm_start);
  out << "found with an algorithm: " << algorithm_result << " in "
  ...
  auto const range_algorithm_duration =
      std::chrono::duration_cast<std::chrono::milliseconds>(range_algorithm_end -
                                                            range_algorithm_start);
  out << "found with a range algorithm: " << range_algorithm_result << " in "
      << range_algorithm_duration.count() << "ms" << std::endl;
}
